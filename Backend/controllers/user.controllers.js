...
const deleteUser = async (req, res, next ) => {
  ...
  try {
    ...
    res.status(200).json({message: 'Success! User has been removed!', deleteUser: deleteUser})
  }
  catch (err) {
    if(!err.statusCode) {
      res.status(500).json({message: 'Something went wrong, try again later!'})
...
const updateUser = async (req, res, next ) => {
  try {
    const userId = req.params.id;
    const UpdateUser = await UserSchema.findByIdAndUpdate(userId, req.body, { new: true }); // an optional third parameter that specifies that you want to receive the updated user information in the response instead of the old information
  
    if(!UpdateUser) {
      res.status(404).json({message: "User doesn't exists"})
    }

    res.status(200).json({message: 'User has been updated', UpdateUser: UpdateUser})
  } 
  catch (err) {
    if(!err.statusCode) {
      res.status(500).json({message: "Something went wrong, try again later!"})
...
const signup = async (req, res, next) => {
    ...
    // object desctructor
    const { firstName, surname, email, password, jobTitle, image } = req.body;
    ...
    if (existingUser) {
      res.status(422).json({message: 'User already exists, please login instead.'})
    }
...
const login = async (req, res, next) => {
        ...
        if (!existingUser) {
          res.status(403).json({message: 'Invalid credentials, could not log you in.'})
        }
